...
function setupChat() {
  ...
  // 发送消息（优先走 Vercel 后端代理 /api/chat，避免 CORS 问题）
  function sendMessage() {
    ...
    if (!apiKey) {
      addMessage('ai', '⚠️ 请先配置豆包 API Key。\n\n点击左下角「⚙️ API 设置」，填入你的火山引擎 API Key（在 console.volcengine.com 获取）。');
      scrollMessages();
      return;
    }
    ...
    // 判断走哪种模式
    // 1. 如果当前页面是 Vercel 部署（有 /api/chat），优先走后端代理
    // 2. 如果配置了公开 CORS 代理 URL，走代理拼接
    // 3. 否则尝试直连（可能 CORS 报错）
    var isVercelDeploy = window.location.hostname.indexOf('vercel.app') !== -1 ||
                         window.location.hostname.indexOf('localhost') !== -1;

    var fetchPromise;

    if (isVercelDeploy && !proxyUrl) {
      // 走 Vercel 后端代理（/api/chat），由服务端转发，无 CORS 问题
      ...
      // 走用户配置的公开 CORS 代理（拼接模式）
      fetchPromise = fetch(proxyUrl + encodeURIComponent(apiEndpoint), {
        method: 'POST',
        headers: {
    ...
    .then(function(response) {
      ...
      if (!response.ok) {
        return response.text().then(function(err) {
          ...
          var detail = (parsed && parsed.detail) ? parsed.detail : err.slice(0, 200);
          var msg = '❌ 请求失败（' + response.status + '）：' + detail;
          if (response.status === 504) {
            msg = '⏱️ 请求超时，豆包 API 响应太慢，请稍后重试。';
          } else if (response.status === 401 || response.status === 403) {
            msg = '🔑 API Key 无效或无权限，请检查 API Key 是否正确。';
          }
          addMessage('ai', msg);
        });
      }
      ...
      return response.json().then(function(data) {
        ...
        if (data && data.choices && data.choices[0] && data.choices[0].message) {
          ...
          reply = '❌ API 错误：' + (data.error.message || JSON.stringify(data.error));
        } else {
          reply = '（未收到有效回复，请检查模型名称是否正确）';
        }
        addMessage('ai', reply);
      });
    })
    .catch(function(e) {
      ...
      if (typingEl) typingEl.remove();
      var errMsg;
      if (e.message && (e.message.indexOf('Failed to fetch') !== -1 || e.message.indexOf('NetworkError') !== -1)) {
        if (isVercelDeploy) {
          errMsg = '❌ 后端代理请求失败，请确认 Vercel 的 /api/chat 函数已部署成功。\n\n可前往 Vercel 控制台查看 Functions 日志。';
        } else {
          errMsg = '❌ CORS 跨域拦截。\n\n解决方法：\n1. 将网页部署到 Vercel（自动启用 /api/chat 代理）\n2. 或在「⚙️ API 设置」填入 CORS 代理地址';
        }
      } else {
